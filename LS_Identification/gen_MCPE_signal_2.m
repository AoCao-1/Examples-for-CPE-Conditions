function u_all = gen_MCPE_signal_2(m, T_i, L, N, u_inil, sv)
%% 3. Calculate Δ_i
...
for i = 2:N
    ...
    if i == 1
        ...
        if L-1 >= 1
            u_all{i}(:, 1:L-1) = sv * randn(m, L-1); % Randomly generate the first L-1 control inputs
        end
        % Set u_all{i}(:, L) as a specific vector to ensure rank=1
        u_all{i}(:, L) = sv * rands(m,1); % Example: first input is 1, others are 0
    else
        % For i > 1 trajectories
        if Delta_i >= 1
            u_all{i}(:, 1:Delta_i) = sv * randn(m, Delta_i); % Randomly generate the first Delta_i control inputs
        end
...
for i = 2:N
    % Ensure that control inputs not in {L-1, 2L-1, ..., mL-1} positions are zero
    for k = L + 1:T_i(i)
